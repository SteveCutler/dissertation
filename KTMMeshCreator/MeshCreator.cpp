...
/// <param name="hInstance">handle to the application instance</param>
/// <param name="nCmdShow">whether to display minimized, maximized, or normally</param>
int MeshCreator::Run(HINSTANCE hInstance, int nCmdShow){
    MSG       msg = {0};
    WNDCLASS  wc;
    ...
    HWND hWndApp = CreateDialogParamW(
        ...
        NULL,
        (DLGPROC)MeshCreator::MessageRouter, 
...
void MeshCreator::Update(){
	...
	if(NULL != kinect){
		...
		kinect->getDepthResolution(dWidth, dHeight);
		m_pDrawDepth->DrawDepth(depthData, dWidth * dHeight);


		kinect->getRGBResolution(RGBWidth, RGBHeight);
		m_pDrawRGB->DrawRGB(RGBData, RGBWidth * RGBHeight * OUT_FRAME_CHANNELS);

		pointCloud.addToCloud(depthData, dWidth, dHeight);
	}
	}catch(...){
...
LRESULT CALLBACK MeshCreator::MessageRouter(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    MeshCreator* pThis = NULL;
    
    if (WM_INITDIALOG == uMsg)
    {
        pThis = reinterpret_cast<MeshCreator*>(lParam);
        SetWindowLongPtr(hWnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(pThis));
    }
    else
    {
        pThis = reinterpret_cast<MeshCreator*>(::GetWindowLongPtr(hWnd, GWLP_USERDATA));
    }
...
/// <param name="lParam">additional message data</param>
/// <returns>result of message processing</returns>
LRESULT CALLBACK MeshCreator::DlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
        case WM_INITDIALOG:
...
void MeshCreator::SetStatusMessage(WCHAR * szMessage)
{
    //SendDlgItemMessageW(m_hWnd, IDC_STATUS, WM_SETTEXT, 0, (LPARAM)szMessage);
	KTM::NotificationInterface::setMessage(szMessage);
}

std::string MeshCreator::getFilePath(wchar_t* dialogTitle){
	OPENFILENAME ofn;
...
std::string MeshCreator::getFilePath(){
	return getFilePath(L"Open...");
}

void MeshCreator::changeRGBResolution(NUI_IMAGE_RESOLUTION resolutionCode){
...
void MeshCreator::changeDepthResolution(NUI_IMAGE_RESOLUTION resolutionCode){
	switch(resolutionCode){
	case NUI_IMAGE_RESOLUTION_640x480:
...
void MeshCreator::resetDepthImageRenderer(){
	int width;
	int height;
...
void MeshCreator::resetRGBImageRenderer(){
	int width;
	int height;
...
void MeshCreator::enableResolutionButtons(bool enabled){
...
void MeshCreator::updateResolutionButtons(){
